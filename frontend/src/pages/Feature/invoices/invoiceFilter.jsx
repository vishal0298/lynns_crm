...
const InvoiceFilter = ({
  ...
  const onSearchChange = async (val, type) => {
    if (type == "CUSTOMER") {
      ...
      if (val !== "") {
        ...
        try {
          let searchUrl = listcustomerApi;
          if (val !== "") searchUrl = `${searchUrl}?address=${val}`;
          const response = await getData(searchUrl, false);
  ...
  //     try {
  //       console.log(key6)
  //         const villaQuery = `address=${key6.join(",")}`;
                              ...
                              return (
                                ...
                                >
                                  <input
                                    ...
                                    defaultChecked={false}
                                    onChange={(e) =>
                                      handleCheckboxChange(e, item?.address, "VILLA")
                                    }
                                  />
                                  <span className="checkmark" /> {item.address}
                                </label>
                              );
                            })}
                          {noData && <span>Adress Not Found !</span>}
                        </div>
                      </div>
                    </div>
                  </div>
